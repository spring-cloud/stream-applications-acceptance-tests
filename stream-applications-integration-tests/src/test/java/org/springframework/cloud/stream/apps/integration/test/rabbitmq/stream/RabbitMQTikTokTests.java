...
import org.springframework.cloud.stream.apps.integration.test.rabbitmq.support.RabbitMQStreamIntegrationTestSupport;

import static org.awaitility.Awaitility.await;
import static org.springframework.cloud.stream.app.test.integration.kafka.KafkaStreamApps.kafkaStreamApps;

public class RabbitMQTikTokTests extends RabbitMQStreamIntegrationTestSupport {

	private static LogMatcher logMatcher = LogMatcher.matchesRegex(".*\\d{2}/\\d{2}/\\d{2}\\s+\\d{2}:\\d{2}:\\d{2}")
			.times(3);

	@Container
	private static final StreamApps streamApp = kafkaStreamApps(RabbitMQTikTokTests.class.getSimpleName(), rabbitmq)
			.withSourceContainer(defaultRabbitMQContainerFor("time-source"))
			.withSinkContainer(defaultRabbitMQContainerFor("log-sink").withLogConsumer(logMatcher).log())
			.build();
