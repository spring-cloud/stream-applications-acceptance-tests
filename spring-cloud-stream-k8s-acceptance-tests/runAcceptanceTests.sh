...
function prepare_ticktock_latest_with_kafka_binder() {
    ...
    kubectl create -f templates/kafka-zk-deployment.yaml
    kubectl create -f templates/kafka-zk-svc.yaml

    kubectl create -f templates/kafka-deployment.yaml
    kubectl create -f templates/kafka-svc.yaml

    kubectl create -f templates/time.yaml
    kubectl create -f templates/time-svc-lb.yaml

    kubectl create -f templates/log.yaml
    kubectl create -f templates/log-svc-lb.yaml
...
./mvnw clean package -Dtest=TickTockK8SAcceptanceTests -Dmaven.test.skip=false -Dtime.source.route=$FULL_TICKTOCK_TIME_SOURCE_ROUTE -Dlog.sink.route=$FULL_TICKTOCK_LOG_SINK_ROUTE
BUILD_RETURN_VALUE=$?
